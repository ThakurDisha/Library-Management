...
//CHANGES MADE BELOW
//Added the function below to get the author related to the book by entering the book ID.
router.get("/authors/:bookID", function(req, res) {
  db.Book.findByPk(req.params.bookID, {include: [db.Author]}).then(function(book) {
    if (book) {
      ret.json([book], res);
    } else {
...
//CHANGES MADE BELOW
//Added the Quantity field for books in the database.
router.post("/", function(req, res) {
    db.Book.create({ title: req.body.title, isbn: req.body.isbn, quantity: req.body.quantity}).then(function(book) {
        ret.json(book, res);
    });
});
...
router.post("/:bookID/authors", function(req, res) {
    db.Book.findByPk(req.params.bookID, { include: [db.Author] }).then(function(book) {
        if (book) {
            db.Author.findOrCreate({ where: { name: req.body.name, website: req.body.website } }).spread(function(
                author,
                created
            ) {
